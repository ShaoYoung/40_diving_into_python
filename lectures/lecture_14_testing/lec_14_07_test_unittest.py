# Основы тестирования
# сравнение doctest и unittest

# ввод-вывод информации
import io
import unittest
# для расширения функционала
from unittest.mock import patch
from lec_14_06 import is_prime


class TestPrime(unittest.TestCase):
    # тестовые кейсы
    def test_is_prime(self):
        # ожидаем False
        self.assertFalse(is_prime(42))
        # ожидаем True
        self.assertTrue(is_prime(73))

    def test_type(self):
        # ожидаем ошибку (ошибка, функция, значения)
        self.assertRaises(TypeError, is_prime, 3.14)

    def test_value(self):
        # ожидаем ошибки для нескольких случаев
        with self.assertRaises(ValueError):
            is_prime(-100)
            is_prime(1)

    # для обработки предупреждающего сообщения для больших чисел. Перехватываем поток ввода-вывода в mock_stdout
    @patch('sys.stdout', new_callable=io.StringIO)
    def test_warning_false(self, mock_stdout):
        self.assertFalse(is_prime(100_000_001))
        # Проверяем перехваченный поток на равенство с текстом сообщения
        self.assertEqual(mock_stdout.getvalue(),
                         'If the number P is prime, the check may take a long time. Working...\n')

    @patch('sys.stdout', new_callable=io.StringIO)
    def test_warning_true(self, mock_stdout):
        self.assertTrue(is_prime(100_000_007))
        self.assertEqual(mock_stdout.getvalue(),
                         'If the number P is prime, the check may take a long time. Working...\n')


if __name__ == '__main__':
    unittest.main(verbosity=2)
